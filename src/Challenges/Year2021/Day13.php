...
class Day13 extends ChallengeBase
{
    ...
    private function print()
    {
        for ($y = 0; $y <= $this->h; ++$y) {
            for ($x = 0; $x <= $this->w; ++$x) {
                if (isset($this->paper[$y][$x]) && 1 == $this->paper[$y][$x]) {
                    echo '#';
                } else {
                    echo '.';
                }
    ...
    private function getCharAt($position)
    {
        ...
        for ($y = 0; $y <= $this->h; ++$y) {
            for ($x = 0; $x < $this->charWidth; ++$x) {
                if (isset($this->paper[$y][$position * $this->charWidth + $x]) && 1 == $this->paper[$y][$position * $this->charWidth + $x]) {
                    $char .= '#';
                } else {
                    $char .= '.';
                }
    ...
    private function count()
    {
        ...
        for ($y = 0; $y <= $this->h; ++$y) {
            for ($x = 0; $x <= $this->w; ++$x) {
                $score += (isset($this->paper[$y]) && isset($this->paper[$y][$x]) && 1 == $this->paper[$y][$x]) ? $this->paper[$y][$x] : 0;
            }
        }
